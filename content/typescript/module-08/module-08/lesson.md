...
- Comprendre les notions essentielles de **Union & Intersection**.
...
Union & Intersection doit être abordé comme un contrat entre le code, le compilateur et les personnes qui le maintiennent. Commence par modéliser les données, préfère les types explicites aux hypothèses implicites et garde les frontières d'exécution sous contrôle.
...
const example: ModuleExample = { title: 'Union & Intersection', ready: true };
...
1. Crée une représentation typée de **Union & Intersection**, puis écris une fonction qui refuse une donnée invalide.
   ...
   const item: Item = { name: 'Union & Intersection', valid: true };
   ```
   :::
