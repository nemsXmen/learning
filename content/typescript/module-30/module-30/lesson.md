...
- Comprendre les notions essentielles de **TypeScript & NPM**.
...
TypeScript & NPM doit être abordé comme un contrat entre le code, le compilateur et les personnes qui le maintiennent. Commence par modéliser les données, préfère les types explicites aux hypothèses implicites et garde les frontières d'exécution sous contrôle.
...
const example: ModuleExample = { title: 'TypeScript & NPM', ready: true };
...
1. Crée une représentation typée de **TypeScript & NPM**, puis écris une fonction qui refuse une donnée invalide.
   ...
   const item: Item = { name: 'TypeScript & NPM', valid: true };
   ```
   :::
