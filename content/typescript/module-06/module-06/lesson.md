...
- Comprendre les notions essentielles de **Type Aliases**.
...
Type Aliases doit être abordé comme un contrat entre le code, le compilateur et les personnes qui le maintiennent. Commence par modéliser les données, préfère les types explicites aux hypothèses implicites et garde les frontières d'exécution sous contrôle.
...
const example: ModuleExample = { title: 'Type Aliases', ready: true };
...
1. Crée une représentation typée de **Type Aliases**, puis écris une fonction qui refuse une donnée invalide.
   ...
   const item: Item = { name: 'Type Aliases', valid: true };
   ```
   :::
