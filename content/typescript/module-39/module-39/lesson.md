...
- Comprendre les notions essentielles de **Clean Architecture**.
...
Clean Architecture doit être abordé comme un contrat entre le code, le compilateur et les personnes qui le maintiennent. Commence par modéliser les données, préfère les types explicites aux hypothèses implicites et garde les frontières d'exécution sous contrôle.
...
const example: ModuleExample = { title: 'Clean Architecture', ready: true };
...
1. Crée une représentation typée de **Clean Architecture**, puis écris une fonction qui refuse une donnée invalide.
   ...
   const item: Item = { name: 'Clean Architecture', valid: true };
   ```
   :::
