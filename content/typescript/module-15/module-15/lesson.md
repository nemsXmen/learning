...
- Comprendre les notions essentielles de **Classes**.
...
Classes doit être abordé comme un contrat entre le code, le compilateur et les personnes qui le maintiennent. Commence par modéliser les données, préfère les types explicites aux hypothèses implicites et garde les frontières d'exécution sous contrôle.
...
const example: ModuleExample = { title: 'Classes', ready: true };
...
1. Crée une représentation typée de **Classes**, puis écris une fonction qui refuse une donnée invalide.
   ...
   const item: Item = { name: 'Classes', valid: true };
   ```
   :::
