...
- Comprendre les notions essentielles de **TypeScript & Async**.
...
TypeScript & Async doit être abordé comme un contrat entre le code, le compilateur et les personnes qui le maintiennent. Commence par modéliser les données, préfère les types explicites aux hypothèses implicites et garde les frontières d'exécution sous contrôle.
...
const example: ModuleExample = { title: 'TypeScript & Async', ready: true };
...
1. Crée une représentation typée de **TypeScript & Async**, puis écris une fonction qui refuse une donnée invalide.
   ...
   const item: Item = { name: 'TypeScript & Async', valid: true };
   ```
   :::
