...
- Comprendre les notions essentielles de **Enums**.
...
Enums doit être abordé comme un contrat entre le code, le compilateur et les personnes qui le maintiennent. Commence par modéliser les données, préfère les types explicites aux hypothèses implicites et garde les frontières d'exécution sous contrôle.
...
const example: ModuleExample = { title: 'Enums', ready: true };
...
1. Crée une représentation typée de **Enums**, puis écris une fonction qui refuse une donnée invalide.
   ...
   const item: Item = { name: 'Enums', valid: true };
   ```
   :::
